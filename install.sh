...
echo ""
echo "  Optional — deep file monitoring (captures file *opens/reads* + the process; Linux, needs root):"
echo "    set ${CYAN}deep_monitor = \"fanotify\"${NC} under [sensors.filesystem] in config/default.toml, then run:"
echo "    ${CYAN}sudo python3 $INSTALL_DIR/tools/capman-fsmon/fsmon.py --backend auto${NC}"
echo "    (see ${CYAN}$INSTALL_DIR/docs/FILE_MONITORING.md${NC})"
echo ""
