...
echo ""
echo "  Optional — deep file monitoring (captures file *opens/reads* + the responsible process; needs root):"
echo "    set ${CYAN}deep_monitor${NC} under [sensors.filesystem] in config/default.toml (Linux: fanotify; macOS: eslogger), then:"
echo "    ${CYAN}sudo python3 $INSTALL_DIR/tools/capman-fsmon/fsmon.py --backend auto${NC}"
echo "    (macOS also needs Full Disk Access for the python binary — see ${CYAN}$INSTALL_DIR/docs/FILE_MONITORING.md${NC})"
echo ""
